...
const GrammarTopicFilters: React.FC<GrammarTopicFiltersProps> = ({
  ...
  const hasActiveFilters = filters.search || filters.levels.length > 0 || filters.categories.length > 0 || filters.difficulty !== 'all';
  return <div className="space-y-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
      {/* Search Bar */}
      <div className="relative">
        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
        <Input placeholder="Search grammar topics..." value={filters.search} onChange={e => updateFilters({
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-100">
        ...
        {hasActiveFilters && <Button variant="ghost" size="sm" onClick={clearAllFilters} className="text-gray-500 hover:text-gray-700">
      ...
      {hasActiveFilters && <div className="flex flex-wrap gap-2">
          ...
          {filters.levels.map(level => <Badge key={level} variant="secondary" className="gap-1">
              {level}
              <X className="h-3 w-3 cursor-pointer" onClick={() => toggleLevel(level)} />
            </Badge>)}
          {filters.categories.map(category => <Badge key={category} variant="secondary" className="gap-1">
              {category}
              <X className="h-3 w-3 cursor-pointer" onClick={() => toggleCategory(category)} />
            </Badge>)}
          {filters.difficulty !== 'all' && <Badge variant="secondary" className="gap-1">
