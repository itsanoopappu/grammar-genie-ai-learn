...
const OptimizedGrammarTopics: React.FC<OptimizedGrammarTopicsProps> = ({
  onTopicSelect
}) => {
  const {
    data: topics = [],
  ...
  const filteredTopics = useMemo(() => {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(topic => topic.name.toLowerCase().includes(searchLower) || topic.description?.toLowerCase().includes(searchLower));
    }
    ...
    if (filters.difficulty !== 'all') {
      filtered = filtered.filter(topic => {
        ...
        switch (filters.difficulty) {
          ...
          case 'medium':
            return difficulty > 30 && difficulty <= 70;
          case 'hard':
            return difficulty > 70;
          default:
            return true;
        }
      });
  ...
  return <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Filters */}
      <GrammarTopicFilters filters={filters} onFiltersChange={setFilters} totalTopics={topics.length} filteredCount={filteredTopics.length} availableCategories={availableCategories} />

      {/* Topics Grid */}
      {filteredTopics.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        return <PracticeCard key={topic.id} title={topic.name} description={topic.description || ''} level={topic.level} difficulty={getDifficultyLevel(topic.difficulty_score)} estimatedTime={getEstimatedTime(topic.difficulty_score)} priority={priority} recommended={isRecommended} reason={reason} variant={isRecommended ? 'ai' : 'topic'} onAction={() => onTopicSelect(topic.id)} actionLabel="Start Practice" className="h-full" />;
      })}
        </div> : <Card className="text-center p-12">
